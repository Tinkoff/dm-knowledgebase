...
* Событие — явление, которое может произойти или не произойти в результате опыта, при сопоставлении предпологаемого результата с фактическим наблюдением
...
Для примера понимания того как работает метод, попробуем его использовать для решения задачи прогнозирования завершения проекта интеллектуальной работы.

Есть несколько важных замечаний при использовании метода для прогнозирования интеллектуальной работы.

Когда мы говорим о прогнозировании сроков интеллектуальной работы команды, например в ИТ, то лучшим отрезком свертки данных, будет является неделя. А связано это с тем, что количество задач выполненных за день будет сильно изменятся от дня в день, по причине высокой волатильности результатов зависящих как от эмоционального состояния людей, так и разной сложности решаемых задач и многих других параметров.

## Метод Монте-Карло для прогнозирования сроков завершения проекта на основе пропускной способности

Для примера необходимо взять достаточный набор данных, предлагаю использовать периоды от 7 до 26 недель.

С чем может быть связано?

7 недель, это чуть более чем полтора месяца, меньше данных для использования метода Монте-Карло приведет высокой степени недостоверности результата.
...
![Chart Throughput](./assets/Monte-Carlo-Througput.png)
...
![Burn Cahrt of Project](./assets/Monte-Carlo-Burndown-chart.jpg)
...
![Burn Cahrt of Project](./assets/Monte-Carlo-Burndown-cahrt-full.jpg)
...
![All Experiments](./assets/Monte-carlo-all-experiments.png)
...
![Table Results](./assets/Monte-Carlo-Table-Results.png)
...
![Table Results](./assets/Monte-Carlo-Table-Results-Full.png)
...
В данном случае можно взять в качестве сроков выполнения проекта **16 неделю**.
...
## Выводы
Даже используя простые модели для моделирования сроков завершения проектов, вы можете при помощи метода Монте-Карло, определить вероятность завершения проекта.
